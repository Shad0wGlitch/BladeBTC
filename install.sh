...
make_install(){
    ...
    #enabling https
    echo -e "\e[92mEnabling HTTPS Firewall ... [PLEASE WAIT]\e[0m"
    ufw allow 'Apache Full'
    echo -e "\e[92mEnabling HTTPS Firewall ... [DONE]\e[0m"
...
check_database_version() {
   ...
   if [[ $(mysql -N -s -u ${DB_USER} -p${DB_PASS} -e \
		...
		DB_VERSION=$(echo "SELECT db_version FROM version WHERE id = 1" | mysql telegram_bot -u ${DB_USER} -p${DB_PASS} -s -N)
